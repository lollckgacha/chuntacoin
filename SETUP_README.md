...
이름/BJ ID가 채워져 있고, `streamer_coin_app.html`의 `STREAMER_LIST`에도 동일한
...
그대로 가져왔습니다. 이미지가 바뀌거나 멤버가 추가/변경되면 `streamer_coin_app.html`의
...
`streamer_coin_app.html`의 `firebaseConfig`와 `.firebaserc`를 천타버스 팡팡(매치 퍼즐
...
에서 확인만 하면 됩니다. `firebaseConfig`는 이미 `streamer_coin_app.html`에 채워져
있습니다 (팡팡과 동일한 프로젝트 값).
...
6. `streamer_coin_app.html`을 Firebase Hosting에 배포하거나 로컬에서 열어서 테스트
